...
func TestConnectorParsesPortalListingCards(t *testing.T) {
	...
	connector := NewConnector(nil, nil)
	tests := []struct {
		name      string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			items, err := connector.Parse(context.Background(), test.source, []byte(test.payload))
			if err != nil {
...
func TestConnectorValidatesConfig(t *testing.T) {
	...
	connector := NewConnector(nil, nil)
	err := connector.ValidateSource(domain.Source{ConfigJSON: `{"item_selector":"article.item","title_selector":"a.item-link"}`})
	if err == nil {
