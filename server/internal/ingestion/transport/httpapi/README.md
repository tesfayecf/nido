<!--
File Name: README.md
Purpose: Provides folder-level documentation for `internal/ingestion/transport/httpapi`.
Responsibilities:
...
Adapts ingestion use cases to authenticated HTTP endpoints.

## Contained components

Source, property, run, analytics, field, and tag handlers.

### Files

- `field_handlers.go`
- `handlers.go`
- `property_handlers.go`
- `tag_handlers.go`

## Interactions with other folders

Depends on auth principal context, ingestion services, and platform HTTP helpers.
...
```mermaid
flowchart LR
    Parent["Parent folder"] --> This["Ingestion HTTP API"]
